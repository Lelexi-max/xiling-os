...
- GitHub Actions hosted 合并门禁覆盖 Ubuntu 与 macOS 的安装、类型检查、测试、Web 构建和平台 smoke；不把 `windows-latest` 当作原生产品运行时。
- 真实 Windows 路径通过可选的 WSL2 Linux 自托管 runner（`xiling-wsl2` 标签）执行；未配置该 runner 时不会让普通 PR 排队。
...
1. Linux/macOS hosted CI 全部通过；若启用 `XILING_WSL2_RUNNER`，WSL2 自托管 job 也必须通过。
...
- 在真实 Windows 11 x86_64 + WSL2 + Docker Desktop 专机完成全新安装、中文用户名、C/D 盘、OneDrive、重启、代理、低磁盘与取消恢复矩阵。普通 GitHub Windows runner 不替代 WSL2 验收；若接入 GitHub，自托管 runner 必须在 WSL2 内运行。
