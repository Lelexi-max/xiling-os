...
if ($PSCmdlet.ShouldProcess($item.FullName, "Create a content-addressed snapshot inside Xi Ling OS WSL storage")) {
    wsl.exe --distribution $Distribution --exec /opt/xiling-os/scripts/windows-import.sh $item.FullName $ProjectId
