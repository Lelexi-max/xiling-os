...
try {
  const app = createApp({ dataRoot: root, connectorMode: "fixture" });
  ...
  const project = await app.inject({ method: "POST", url: "/api/gate4/projects", payload: { name: "Gate 4.5-D", description: "ownership smoke", researchQuestion: "Is the formal Agent path durable?" } });
  const projectId = project.json().id;
  const session = await app.inject({ method: "POST", url: "/api/gate4/chat-sessions", payload: { projectId, title: "formal" } });
  const sessionId = session.json().id;
  if (!projectId || !sessionId) throw new Error("Project/session setup failed");
  if ((await app.inject({ method: "POST", url: `/api/gate4/chat-sessions/${sessionId}/messages`, payload: { role: "user", text: "legacy", status: "complete" } })).statusCode !== 404) throw new Error("Retired message writer is still available");
  ...
  const workflow = await app.inject({ method: "POST", url: "/api/gate4/research-workflows", payload: { projectId, sessionId, sourceCallId: "manual-smoke", request } });
  ...
  if ((await app.inject({ method: "POST", url: `/api/gate4/research-workflows/${workflowId}/probe`, payload: { projectId: "ocean-heatwave" } })).statusCode !== 404) throw new Error("Workflow action crossed project scope");
  if ((await app.inject({ method: "POST", url: `/api/gate4/research-workflows/${workflowId}/run`, payload: { projectId } })).statusCode !== 409) throw new Error("Unapproved workflow was executable");
  await app.close();

  const restored = createApp({ dataRoot: root, connectorMode: "fixture" });
  const messages = await restored.inject({ method: "GET", url: `/api/gate4/chat-sessions/${sessionId}/messages` });
