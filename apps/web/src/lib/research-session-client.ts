...
export async function* runResearchTurn(request: ResearchTurnRequest): AsyncGenerator<AgentStreamEvent> {
  ...
  try {
    const response = await fetch(
      `/api/agent-center/runs/${encodeURIComponent(runId)}/events?projectId=${encodeURIComponent(request.projectId)}&afterSequence=0`,
      request.signal ? { signal: request.signal } : undefined,
    );
    for await (const raw of streamAgentEvents(response) as AsyncGenerator<unknown>) {
      const persisted = raw as { type?: string; payload?: unknown };
      ...
      if (persisted.type === "run.completed" || persisted.type === "run.cancelled" || persisted.type === "run.suspended") yield { type: "run.settled", runId, status: persisted.type.slice(4) as "completed" | "cancelled" | "suspended" };
